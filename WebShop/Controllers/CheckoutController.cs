...
namespace WebShop.Controllers
{
    public class CheckoutController : Controller
    {
        ...
        public CheckoutController(dbMarketsContext context, INotyfService notyfService, IVnPayService vnPayService)
        {
            _context = context;
        ...
        public IActionResult Index(MuaHangVM muaHang)
        {
            ...
            var cart = HttpContext.Session.Get<List<CartItem>>("GioHang");
            var taikhoanID = HttpContext.Session.GetString("CustomerId");
            var amountSession = cart.Sum(x => x.TotalMoney);
            muaHang.Amount = (decimal)amountSession;
            var url = _vnPayService.CreatePaymentUrl(muaHang, HttpContext);
            MuaHangVM model = new MuaHangVM();
            if (taikhoanID != null)
            {
                var khachhang = _context.Customers.AsNoTracking().SingleOrDefault(x => x.CustomerId == Convert.ToInt32(taikhoanID));
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    donhang.Paid = false;
                    donhang.Note = UploadHelper.StripHTML(model.Note);
